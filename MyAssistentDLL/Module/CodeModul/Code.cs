...
namespace MyAssistentDLL.Module.Codes
{
    ...
    public static class COMSENDER
    {
        ...
        public static void INIT()
        {
            try
            {
                _serialPort = new SerialPort(MainSettings.COMname, MainSettings.BaudRate, Parity.None, 8, StopBits.One);
    ...
    {
        private readonly static string PATH = @"Saves\SaveCommand.json";
        public static ObservableCollection<CodeSaver> dates_Culture = new ObservableCollection<CodeSaver>();
        public static List<CodeSaver> dates = new List<CodeSaver>();
        internal static int CountCommand { get => dates.Count; }
        internal (string,string) commands;
        internal List<(TypeArgumend, string[], bool local)> Arguments;
        ...
        public static void add((string, string) command, List<(TypeArgumend, string[], bool local)> arguments)
        {
            ...
            var Codes = new CodeSaver() { Arguments = arguments, commands = command };
            dates.Add(Codes);
            dates_Culture.Add(Codes);
            interpretatot.values.Add(command, new Code(arguments));
            Sound.Sound.add_string(command.Item1);
        }
            ...
            public async void Start()
            {
                foreach(var Arguments in Arguments)
                {
                    if (!Arguments.local)
                    {
                       ...
                       Internet.InternetWorkerModule.SendToClent(Arguments.Item2[0], JsonConvert.SerializeObject(new SendToPC(Arguments.Item1.ToString(),Arguments.Item2)));
                        continue;
                    }
                    switch (Arguments.Item1)
                    {
                        ...
                        case TypeArgumend.Voice:
                            foreach (var item in Arguments.Item2)
                                Sound.Voice.PlayRuAsync(BuildSoundString(item));
                            break;
                        case TypeArgumend.Arduino:
                            Internet.InternetWorkerModule.SendToClent(Arguments.Item2[0], Arguments.Item2[1]);
 
                            break;
                        case TypeArgumend.ArduinoReadDate:
                            await System.Threading.Tasks.Task.Run(() =>
                            {
                                if (Internet.InternetWorkerModule.SendToClinetRecive(Arguments.Item2[0], Arguments.Item2[1], out string output))
                                {
                                    if (Arguments.Item2[2].Length != 0 &&
                                        IneterpretatorArduinoDate(Arguments.Item2[2], output, out string voices))
                                        Sound.Voice.PlayRuAsync(voices);
                                }
                            });
        ...
        {
            Sound.Sound.Change_Culture(culture);
           MainSettings.SpeechCulture = culture;
            dates_Culture.Clear();
            LoadGrammar();
        ...
        private static void LoadGrammar()
        {
            ...
            Sound.Sound.ClearItems();
            foreach (var i in dates)
            {
                if (i.commands.Item2.ToLower() == MainSettings.SpeechCulture.ToLower())
                {
                    Sound.Sound.add_string(i.commands.Item1);
        ...
        {
            dates = File.Exists(PATH)?JsonConvert.DeserializeObject<List<CodeSaver>>(File.ReadAllText(PATH)):new List<CodeSaver>();
            LoadGrammar();
        }
        public static void Save()
        {
            if (!Directory.Exists(PATH)) Directory.CreateDirectory(PATH.Split('\\')[0]);
            File.WriteAllText(PATH, JsonConvert.SerializeObject(dates));
        }
         
    }

    internal class interpretatot
    {
        public static Dictionary<(string, string), CodeSaver.Code> values = new Dictionary<(string, string), CodeSaver.Code>();

        

        public static void Run(string command)
        {
            if (values.ContainsKey((command,MyAssistentDLL.MainSettings.SpeechCulture)))
            {
                values[(command, MainSettings.SpeechCulture)].Start();
                Logs.Log.Write(Logs.TypeLog.Message, $"{MainSettings.CultureText[MainSettings.SpeechCulture]} |{command}|");
                if(MainSettings.VoiceMessage&&!MainSettings.VoiceLog)
                    Sound.Voice.PlayRuAsync($"{MainSettings.CultureText[MainSettings.SpeechCulture]} {command}");
            }
